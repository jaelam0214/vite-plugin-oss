...
import { defaultOption } from './type';
// import ora from 'ora'
...
import fs from 'fs';
export default function assetUploaderPlugin(options) {
    const oss = new OSS({
    ...
    const { from, dist, deleteOrigin, deleteEmptyDir, setOssPath, timeout, verbose, test, overwrite, bail, quitWpOnError, } = Object.assign(defaultOption, options);
    /**
     * 上传文件
    ...
    const upload = async (files, inVite, outputPath = '') => {
        ...
        for (let file of _files) {
            ...
            // 为每个文件设置上传的绝对路径
            let ossFilePath = slash(path.join(dist, (setOssPath && setOssPath(filePath)
